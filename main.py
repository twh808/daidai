...
class DaidaiManagerPlugin(Star):
    ...
    # ---------- 核心处理函数：获取环境变量列表 ----------
    async def _handle_env_list(self, event: AstrMessageEvent):
        try:
    ...
    # ---------- 为每个别名单独定义一个指令方法，全部调用同一处理函数 ----------
    @filter.command("envlist")
    async def envlist(self, event: AstrMessageEvent):
        await self._handle_env_list(event)

    @filter.command("环境变量列表")
    async def envlist_zh(self, event: AstrMessageEvent):
        await self._handle_env_list(event)

    @filter.command("变量列表")
    async def envlist_var(self, event: AstrMessageEvent):
        await self._handle_env_list(event)

    @filter.command("envs")
    async def envlist_envs(self, event: AstrMessageEvent):
        await self._handle_env_list(event)

    @filter.command("变量")
    async def envlist_var_short(self, event: AstrMessageEvent):
        await self._handle_env_list(event)
