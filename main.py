...
/更新变量 <变量名> <新值> 或 /更新环境变量 <变量名> <新值> → 直接更新
/交互更新 → 分步引导更新环境变量（三步）
/交互列表 → 查看环境变量列表
...
/菜单、/menu → 显示本菜单"""
    ...
    # ---------- 交互更新（三步指令） ----------
    @filter.command("交互更新")
    async def interactive_update(self, event: AstrMessageEvent, *args):
        """交互式更新环境变量：/交互更新 [变量名] [新值]"""
        user_id = str(event.get_sender_id())
        if len(args) == 0:
            # 第一步：提示输入变量名
            self.temp[user_id] = {'step': 'env_name'}
            yield event.plain_result("📝 请发送：/交互更新 <变量名>")
        elif len(args) == 1:
            # 第二步：存储变量名，提示输入新值
            env_name = args[0]
            self.temp[user_id] = {'step': 'new_value', 'env_name': env_name}
            yield event.plain_result(f"📝 请发送：/交互更新 {env_name} <新值>")
        elif len(args) == 2:
            # 第三步：执行更新
            env_name = args[0]
            new_value = args[1]
            try:
                ...
                if '&' in raw:
                    ...
                    if accounts:
                        msg, count = await self._update_env_accounts(env_name, accounts)
                        yield event.plain_result(f"检测到 {count} 个账户，{msg}" if "✅" in msg else msg)
                    else:
                        yield event.plain_result("❌ 未检测到有效的账户更新条目")
                else:
                    if '#' in raw:
                ...
                if user_id in self.temp:
                    del self.temp[user_id]
        else:
            yield event.plain_result("❌ 参数过多，请检查格式。")

    # ---------- 交互脚本 ----------
    @filter.command("交互脚本")
    async def interactive_script(self, event: AstrMessageEvent, *args):
        """交互式运行脚本：/交互脚本 [脚本路径]"""
        user_id = str(event.get_sender_id())
        if len(args) == 0:
            yield event.plain_result("📝 请发送：/交互脚本 <脚本路径>")
        elif len(args) == 1:
            script_path = args[0]
            try:
                payload = {"path": script_path}
                ...
                if result.get("error") or result.get("code") not in [0, None, ""] or result.get("status") == "error":
                    ...
                    yield event.plain_result("✅ 脚本已成功执行！")
            except Exception as e:
    ...
    # ---------- 交互任务 ----------
    @filter.command("交互任务")
    async def interactive_task(self, event: AstrMessageEvent, *args):
        """交互式运行任务：/交互任务 [任务名称]"""
        if len(args) == 0:
            yield event.plain_result("📝 请发送：/交互任务 <任务名称>")
        elif len(args) == 1:
            task_name = args[0]
            try:
                result = await self._call_api("tasks?page=1&page_size=100", method="GET")
